...
import com.jy.board.movie.MovieDTO;
...
public class ReviewDAO {
    ...
    public static final ReviewDAO RDAO = new ReviewDAO(); // 상수 (private이 아니라 getter 필요 x)
    public Connection con = null;
    // 이부분에 Connection con을 한번에 변수 형성해 줌으로써 아래의 method에
    // 따로 만들 필요가 없어짐

    private ReviewDAO() { //생성자 잠금
        try {
            con = DBManager.connect();
            // 이부분에서 DBManger에서 connect함으로써 아래 method에서 con = DBManager.connect() 부분도 필요없어짐!
    ...
    public void getReview(HttpServletRequest request) {
       // Connection conn = null;
        ...
        try {
       //     con = DBManager.connect();
            pstmt = con.prepareStatement(sql);   /* jsp에서 설정한 parameter */
